...
- **正确做法**：
  ...
  - ✅ 写完一段，用户测试后，再写下一段
  - ✅ 说明这段代码的作用、数据流向
  - ✅ **按功能单元拆分**：模板中用到的变量、函数、样式要和模板一起给出，作为一个完整的功能单元
    - 例如：模板里写了 `<el-button @click="handleSave">`，就同时给出 `handleSave` 函数的代码
    - 例如：模板里用了 `v-model="dialogVisible"`，就同时给出 `dialogVisible` 变量的声明
    - 不要先把整个模板全给完，再单独给整个 script——而是一块功能对应的模板 + 变量 + 函数一起给
...
#### 任务清单
- [已完成] 5.1 创建 categories store（含默认分类数据）
- [已完成] 5.2 实现分类列表页面（收入/支出 Tab 切换）
- [已完成] 5.3 实现新增分类
- [已完成] 5.4 实现编辑分类
- [已完成] 5.5 实现删除分类（校验是否被账单引用）
- [跳过] 5.6 图标选择功能（暂用固定图标）
...
#### 任务清单
- [已完成] 7.1 创建 budget store
- [已完成] 7.2 实现预算设置页面
- [已完成] 7.3 实现预算进度展示（Progress 组件）
- [已完成] 7.4 实现超支预警逻辑
...
| 阶段 | 状态 | 备注 |
|------|------|------|
| 阶段一：项目初始化 | 已完成 | 类型定义、工具函数、Element Plus、ECharts 配置 |
| 阶段二：路由与布局 | 已完成 | MainLayout、侧边栏、顶栏、面包屑、路由守卫 |
| 阶段三：登录功能 | 已完成 | 登录页面、表单验证、user store |
| 阶段四：账单管理 | 已完成 | CRUD、筛选、分页、bills store、categories store |
| 阶段五：分类管理 | 已完成 | Tab 切换、卡片列表、新增/编辑/删除 |
| 阶段六：图表与仪表盘 | 已完成 | 仪表盘、统计报表、折线图/饼图/柱状图，剩余图表响应式适配 |
| 阶段七：预算管理 | 已完成 | budget store、预算设置弹窗、Progress 进度条、超支预警 |
| 阶段八：设置与收尾 | 进行中 | 个人信息编辑、数据导出、清空数据已完成，剩余样式优化与代码整理 |
